/* global ajaxurl, wpLinkL10nPopup, setUserSetting */
var wpLinkPopup;
...
(function($){
	...
	wpLinkPopup = {
		...
		beforeOpen : function() {
			...
			// $.trigger( 'beforeopen.wplinkchooser' );
		...
		keyup: function( event ) {
			...
			if ( event.which === key.ESCAPE ) {
				...
				if ( ! $(document).triggerHandler( 'wp_CloseOnEscape', [{ event: event, what: 'wplink', cb: wpLinkPopup.close }] ) ) {
					wpLinkPopup.close();
				}
				return false;
			}
		...
		toggleInternalLinking : function( event ) {
			var panel = $('#wp-link-chooser .search-panel'),
	...
	$.extend( River.prototype, {
		...
		ajax: function( callback ) {
			var self = this,
				delay = this.query.page === 1 ? 0 : wpLinkPopup.minRiverAJAXDuration,
				response = wpLinkPopup.delayedCallback( function( results, params ) {
			...
			if ( !results ) {
				if ( firstPage ) {
					list += '<li class="unselectable"><span class="item-title"><em>' +
						wpLinkL10nPopup.noMatchesFound + '</em></span></li>';
				}
			} else {
